...
rl_propertyAttributes *rl_copyPropertyAttributes (objc_property_t property) {
    ...
    rl_propertyAttributes *attributes = calloc(1, sizeof(rl_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate rl_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = rl_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = rl_propertyMemoryManagementPolicyRetain;
            break;
...
Method rl_getImmediateInstanceMethod (Class aClass, SEL aSelector) {
